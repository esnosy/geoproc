...
int main(int argc, char **argv)
{
  ...
  std::mt19937 gen(rd());
  std::discrete_distribution<size_t> dd(triangle_areas.begin(),
                                        triangle_areas.end());
  ...
  ofs << "end_header\n";
  ofs.write(reinterpret_cast<char *>(&output_points[0]),
            12 * output_points.size());
  const auto t7 = std::chrono::high_resolution_clock::now();
  std::cout
      << "Writing output file took: "
